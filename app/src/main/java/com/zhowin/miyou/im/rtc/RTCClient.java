package com.zhowin.miyou.im.rtc;
...
import com.zhowin.miyou.im.adapter.RTCEventsListenerAdapter;
import com.zhowin.miyou.im.constant.SealMicConstant;
import com.zhowin.miyou.im.manager.CacheManager;
import com.zhowin.miyou.im.manager.RoomManager;
...
public class RTCClient {
    ...
    public void init() {
        //使用默认配置，直接传null
        RCRTCEngine.getInstance().init(BaseApplication.getInstance(), null);
    }
        ...
        RCRTCEngine.getInstance().joinRoom(roomId, rtcRoomType, new IRCRTCResultDataCallback<RCRTCRoom>() {
            ...
            public void onSuccess(final RCRTCRoom rcrtcRoom) {
                    ...
                    public void onRemoteUserPublishResource(RCRTCRemoteUser rcrtcRemoteUser, List<RCRTCInputStream> list) {
                        ...
                        rcrtcRoom.getLocalUser().subscribeStreams(list, new IRCRTCResultCallback() {
                            ...
                            public void onSuccess() {
                                Log.e(TAG, "主播订阅远端用户成功");
                            }

                            @Override
                            public void onFailed(RTCErrorCode rtcErrorCode) {
                                Log.e(TAG, "主播订阅远端用户失败");
                            }
                        });
                    }
                });
                ...
                rcrtcRoom.getLocalUser().publishLiveStream(rcrtcRoom.getLocalUser().getDefaultAudioStream(), new IRCRTCResultDataCallback<RCRTCLiveInfo>() {
                    ...
                    @Override
                    public void onFailed(RTCErrorCode rtcErrorCode) {
                        Log.e(TAG, "主播加入RTC房间之后发布流失败: " + rtcErrorCode.getReason());
                    }
                });
                ...
                for (RCRTCRemoteUser rcrtcRemoteUser : rcrtcRemoteUserList) {
                    rcrtcRoom.getLocalUser().subscribeStreams(rcrtcRemoteUser.getStreams(), new IRCRTCResultCallback() {
                        @Override
                        public void onSuccess() {
                            Log.e(TAG, "订阅当下其他主播的流成功");
                        }

                        @Override
                        public void onFailed(RTCErrorCode rtcErrorCode) {
                            Log.e(TAG, "订阅当下其他主播的流失败");
                        }
                    });
                }
            }
            ...
            @Override
            public void onFailed(RTCErrorCode rtcErrorCode) {
                Log.e(TAG, "主播加入RTC房间失败: " + rtcErrorCode.getReason());
            }
        });
    }
    ...
    public void micQuitRoom(String roomId) {
        RCRTCEngine.getInstance().leaveRoom(new IRCRTCResultCallback() {
            ...
            @Override
            public void onFailed(RTCErrorCode rtcErrorCode) {
                Log.e(TAG, "主播退出RTC失败: " + rtcErrorCode.getValue());
            }
        });
    }
