package com.zhowin.miyou.im.provider;
...
import com.zhowin.miyou.im.message.ContactNotificationMessage;
...
@ProviderTag(messageContent = io.rong.message.ContactNotificationMessage.class, showPortrait = false, centerInHorizontal = true, showProgress = false, showSummaryWithName = false)
public class ContactNotificationMessageProvider extends IContainerItemProvider.MessageProvider<io.rong.message.ContactNotificationMessage> {
    @Override
    public void bindView(View v, int position, io.rong.message.ContactNotificationMessage content, UIMessage message) {
        ViewHolder viewHolder = (ViewHolder) v.getTag();
        if (content != null) {
            if (!TextUtils.isEmpty(content.getExtra())) {
                ContactNotificationMessage bean = null;
                try {
                    Gson gson = new Gson();
                    bean = gson.fromJson(content.getExtra(), ContactNotificationMessage.class);
    ...
    @Override
    public Spannable getContentSummary(io.rong.message.ContactNotificationMessage content) {
        return null;
    }

    @Override
    public Spannable getContentSummary(Context context, io.rong.message.ContactNotificationMessage content) {
    ...
    @Override
    public void onItemClick(View view, int position, io.rong.message.ContactNotificationMessage
            content, UIMessage message) {
    }

    @Override
    public void onItemLongClick(View view, int position, io.rong.message.ContactNotificationMessage content, final UIMessage message) {
        String[] items;
