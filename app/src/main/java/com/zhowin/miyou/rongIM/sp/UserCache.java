package com.zhowin.miyou.rongIM.sp;
...
public class UserCache {
    ...
    public void saveUserCache(UserCacheInfo userCacehInfo) {
        if (userCacehInfo == null || TextUtils.isEmpty(userCacehInfo.getUserId())) {
            return;
        }
        UserCacheInfo tmpCacheInfo = getUserCache();
        if (tmpCacheInfo != null && !TextUtils.isEmpty(tmpCacheInfo.getUserId()) && !userCacehInfo.getUserId().equals(tmpCacheInfo.getUserId())) {
            // 另一个不同的用户
            Gson gson = new Gson();
